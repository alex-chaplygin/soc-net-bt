...
namespace СоцСеть {
	...
	static string Обработать(string сообщение)
	{
		...
		else if (команда[0] == "Новости")
                    ответ = Новости(Convert.ToInt32(команда[1]), Convert.ToInt32(команда[2]));
		else
	...
	static string Список(int код, string имя, int смещение)
	    ...
	    if (!активныеПользователи.ContainsKey(код))
            ...
            string сообщения = "";
            foreach (Чат ч in соцСеть.ПолучитьЧаты())
                if ((ч.ПолучитьПользователя2().ПолучитьИмя() == имя && ч.ПолучитьПользователя1().ПолучитьИмя() == активныеПользователи[код].ПолучитьИмя()) || (ч.ПолучитьПользователя1().ПолучитьИмя() == имя && ч.ПолучитьПользователя2().ПолучитьИмя() == активныеПользователи[код].ПолучитьИмя()))
            ...
            if (списокСообщений.Count > 0)
            {
                if (списокСообщений.Count > смещение + 10)
                    for (int i = списокСообщений.Count - смещение - 10; i < списокСообщений.Count - смещение; i++)
                        сообщения += $"{списокСообщений[i].ПолучитьПользователя().ПолучитьИмя()}: {списокСообщений[i].ПолучитьТекст()}\n";
                else
	...
	static string Новости(int код, int смещение)
        {
	    if (!активныеПользователи.ContainsKey(код))
                return "Пользователь не авторизован";
            string новости = "";
            List<Сообщение> публикации = new List<Сообщение>();
            List<Пользователь> друзья = активныеПользователи[код].СписокДрузей();
            if (друзья.Count != 0)
                foreach (Пользователь д in друзья)
                {
                    if (д.ПолучитьСтену().ПолучитьПубликации().Count > смещение + 10)
                        for (int i = д.ПолучитьСтену().ПолучитьПубликации().Count - смещение - 10; i < д.ПолучитьСтену().ПолучитьПубликации().Count - смещение; i++)
                            публикации.Add(д.ПолучитьСтену().ПолучитьПубликации()[i]);
                    else if (д.ПолучитьСтену().ПолучитьПубликации().Count > смещение)
                        for (int i = 0; i < д.ПолучитьСтену().ПолучитьПубликации().Count - смещение; i++)
                            публикации.Add(д.ПолучитьСтену().ПолучитьПубликации()[i]);
                    else if (д.ПолучитьСтену().ПолучитьПубликации().Count != 0)
                        for (int i = 0; i < д.ПолучитьСтену().ПолучитьПубликации().Count; i++)
                            публикации.Add(д.ПолучитьСтену().ПолучитьПубликации()[i]);
                    else continue;
                }
            else return "Друзей нет";
            if (публикации.Count > смещение + 10)
                for (int i = публикации.Count - смещение - 10; i < публикации.Count - смещение; i++)
                    новости += $"{публикации[i].ПолучитьПользователя().ПолучитьИмя()}:{публикации[i].ПолучитьТекст()}\n";
            else if (публикации.Count > смещение)
                for (int i = 0; i < публикации.Count - смещение; i++)
                    новости += $"{публикации[i].ПолучитьПользователя().ПолучитьИмя()}:{публикации[i].ПолучитьТекст()}\n";
            else if (публикации.Count != 0)
                for (int i = 0; i < публикации.Count; i++)
                    новости += $"{публикации[i].ПолучитьПользователя().ПолучитьИмя()}:{публикации[i].ПолучитьТекст()}\n";
            if (новости == "")
                return "Новостей нет";
            return новости;
