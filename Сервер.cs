...
namespace СоцСеть {
	...
	static string Обработать(string сообщение)
	{
		...
		else if (команда[0] == "ГруппаПубликация")
                    ответ = ГруппаПубликация(Convert.ToInt32(команда[1]), команда[2], string.Join(" ", команда, 3, команда.Length - 3));
		else
	...
	static string Подписаться(int код, string имя)
        {
		    ...
		    foreach (Пользователь п in соцСеть.НайтиГруппу(имя).ПолучитьУчастников())
			Console.WriteLine("Группа: " + п.ПолучитьИмя());
	...
	static string ГруппаПубликация(int код, string имяГруппы, string текст)
        {
            if (!активныеПользователи.ContainsKey(код))
                return "Пользователь не авторизован";
            Группа группа = соцСеть.НайтиГруппу(имяГруппы);
            if (группа == null)
                return "Группа не найдена";
	    foreach (Пользователь п in группа.ПолучитьУчастников())
		Console.WriteLine("Группа: " + п.ПолучитьИмя());
            if (!группа.ПолучитьУчастников().Contains(активныеПользователи[код]))
                return "Пользователь не состоит в группе";
            соцСеть.НайтиГруппу(имяГруппы).ПолучитьСтену().Опубликовать(new СоцСеть.Сообщение(активныеПользователи[код], текст));
