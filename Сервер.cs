...
namespace СоцСеть {
	...
	static string Сообщение(string номер, string кому, string текст, string время = "")
        {
            ...
            чат.ДобавитьСообщение(new Сообщение(соцСеть.НайтиПользователя(кому), текст, время));
	    ...
	    foreach (Чат ч in чаты)
            {
                ...
                foreach (Сообщение с in сообщения)
                    swСообщения.WriteLine($"{ч.ПолучитьПользователя1().ПолучитьИмя()}:{ч.ПолучитьПользователя2().ПолучитьИмя()}:{с.ПолучитьТекст()}:{с.ПолучитьВремя()}");
            }
            swСообщения.Close();
            foreach (Пользователь п in пользователи)
            {
