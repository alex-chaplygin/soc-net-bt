...
namespace СоцСеть {
	...
	public static string Сообщение(string номер, string кому, string текст)
	{
		...
		if (ч.ПолучитьПользователя1() == активныеПользователи[Convert.ToInt32(номер)] && ч.ПолучитьПользователя2() == соцСеть.НайтиПользователя(кому))
		{
		    чат = ч;
		    flag = true;
		}
	    if (flag == false)
		чат = new Чат(п, соцСеть.НайтиПользователя(кому));
	    for (int i = 0; i < п.ПолучитьЧаты().Count; i++)
		if (чат.ПолучитьПользователя1() == п.ПолучитьЧаты()[i].ПолучитьПользователя1() && чат.ПолучитьПользователя2() == п.ПолучитьЧаты()[i].ПолучитьПользователя2())
		{
		    п.ПолучитьЧаты()[i].ДобавитьСообщение(new Сообщение(соцСеть.НайтиПользователя(кому), текст));
		    return $"Сообщение отправлено: \"{текст}\"";
		}
	    Console.WriteLine(п.ПолучитьЧаты());
