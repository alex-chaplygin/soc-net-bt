...
namespace СоцСеть {
	...
	public static string ОтобразитьСтену(int код)
        {
            string board = "";
            for (int i = активныеПользователи[код].ПолучитьСтену().ПолучитьПубликации().Count - 11; i < активныеПользователи[код].ПолучитьСтену().ПолучитьПубликации().Count; i++)
                board += активныеПользователи[код].ПолучитьСтену().ПолучитьПубликации()[i].ПолучитьТекст() + "\n";
            return board;
        }
    }
}
