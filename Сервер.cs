...
namespace СоцСеть {
	...
	public static string Обработать(string сообщение)
	{
		...
		if (команда[0] == "Регистрация")
		    ...
		    ответ = Сообщение(команда[1], команда[2], String.Join(" ", команда, 3, команда.Length - 3));
                    ...
                    ответ = Список(Convert.ToInt32(команда[1]), команда[2]);
		else
	...
	static string Список(int код, string имя)
        {
            ...
            {
                if (списокСообщений.Count > 10)
                    for (int i = списокСообщений.Count - 11; i < списокСообщений.Count; i++)
                        сообщения += списокСообщений[i].ПолучитьТекст() + "\n";
                else
                    for (int i = 0; i < списокСообщений.Count; i++)
                        сообщения += списокСообщений[i].ПолучитьТекст() + "\n";
                return сообщения;
            }
