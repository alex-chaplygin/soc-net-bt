...
namespace СоцСеть {
	...
	public static void Main(string[] args)
	{
	    ...
	    соцСеть.ДобавитьГруппу("группа1");
	    соцСеть.ДобавитьГруппу("группа2");
	    соцСеть.ДобавитьГруппу("группа3");
	    соцСеть.ДобавитьГруппу("группа4");
	    Загрузить();
	...
	static string Обработать(string сообщение)
	{
		...
		else if (команда[0] == "ПоискГрупп")
                    ответ = ПоискГрупп(Convert.ToInt32(команда[1]), команда[2], Convert.ToInt32(команда[3]));
		else
	...
	static string ПоискГрупп(int код, string шаблон, int смещение)
        {
            List<string> найденныеГруппы = new List<string>();
            if (!активныеПользователи.ContainsKey(код))
                return "Пользователь не авторизован";
            if (соцСеть.ПолучитьГруппы().Count != 0)
            {
                var группы = соцСеть.ПолучитьГруппы();
                foreach (var группа in группы)
                {
                    if (группа.ПолучитьНазвание().Contains(шаблон))
                    {
                        найденныеГруппы.Add(группа.ПолучитьНазвание());
                    }
                }
            }
            string результат = "";
            if (найденныеГруппы.Count > 10 + смещение)
                for (int i = найденныеГруппы.Count - смещение - 10; i < найденныеГруппы.Count - смещение; i++)
                    результат += найденныеГруппы[i] + "\n";
            else
                for (int i = 0; i < найденныеГруппы.Count - смещение; i++)
                    результат += найденныеГруппы[i] + "\n";
            return результат;
        }
    }
