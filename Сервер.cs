...
namespace СоцСеть {
	...
	static string Обработать(string сообщение)
	{
		...
		if (команда[0] == "Регистрация")
		    ...
		    ответ = ОтобразитьСтену(Convert.ToInt32(команда[1]), Convert.ToInt32(команда[2]), Convert.ToInt32(команда[3]));
	...
	static string ОтобразитьСтену(int код, string пользователь, int num)
		...
		return "Пользователь не авторизован";
	    List<Сообщение> сообщения = соцСеть.НайтиПользователя(пользователь).ПолучитьСтену().ПолучитьПубликации();
            string board = "";
