...
namespace СоцСеть {
	...
	static string Сообщение(string time, string номер, string кому, string текст)
        {
            int н;
            Пользователь п;
            ...
            чат.ДобавитьСообщение(new Сообщение(п, текст, time));
		...
		while ((line = srСообщения.ReadLine()) != null)
                {
                    ...
                    Сообщение(time, line.Split(':')[1], line.Split(':')[2], line.Split(':')[3]);
                }
                srСообщения.Close();
	    ...
	    foreach (Чат ч in чаты)
            {
                ...
                foreach (Сообщение с in сообщения)
                    swСообщения.WriteLine($"{с.ПолучитьВремя().Replace(':', ',')}:{ч.ПолучитьПользователя1().ПолучитьИмя()}:{ч.ПолучитьПользователя2().ПолучитьИмя()}:{с.ПолучитьТекст()}");
            }
            swСообщения.Close();
            foreach (Пользователь п in пользователи)
            {
